<template>
  <a-form name="pay" :model="formData" :label-col="{ span: 24 }" ref="formRef">
    <a-row :gutter="16">
      <a-col :span="12">
        <a-form-item :label="t('hourly_rate')" :name="['base', 'amount']">
          <a-input
            v-no-chinese
      ...
      <a-col :span="12">
        <a-form-item :label="t('max_hours')" :name="['base', 'units']">
          <a-input
            v-no-chinese
      ...
      <a-col :span="24">
        <a-form-item :label="t('approximate_hours_to_complete')" name="hours">
          ...
          ></a-input>
          <div class="form-note">
            {{ t("approximate_hours_to_complete_hourly_tip") }}
          </div>
        </a-form-item>
      </a-col>
    </a-row>
...
const temEchoData = () => {
  ...
  if (!currentTemplateData.value?.pay || currentTemplateData.value?.pay.type !== PayTypeEnum.HOURLY)
    return;
