    ...
    <a-collapse-panel key="1" :header="taskItemProps.header">
      ...
      <template #extra>
        <div class="action flex full">
          ...
          <HolderOutlined class="action-sort" />
        </div>
      </template>
...
const props = defineProps<{ taskItemProps: Props }>();
...
const del = () => {
  emits("removeTask", props.taskItemProps.taskId);
};

// task自动展开
const autoUnfolding = () => {
  if (props.taskItemProps.collapsible === "disabled") return;
  activeKey.value = ["1"];
};

const outPutHandle = (value: FormData, validateStatus: boolean) => {
  value.id = props.taskItemProps.taskId;
  value.group_id = props.taskItemProps.group_id;
